...
| Method | Path | Auth | Description |
|---|---|---|---|
| `GET` | `/audit-log` | Admin, Sales Rep, Sales Manager | Filters: `entity_type`, `entity_id`, `actor_id`, date range — full, unrestricted use of all of these is **Admin-only**. Must be **append-only** at the storage layer (`NFR-007`) — no `PUT`/`DELETE` route should exist for this resource at all. |
