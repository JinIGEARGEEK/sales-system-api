...
| **ทีม Marketing** | เพิ่มเมื่อ 2026-09-01 สำหรับ Prospect (§3a) — CRUD เต็มรูปแบบบน Prospects ที่ตนรับผิดชอบหรือยังไม่มีผู้รับผิดชอบ ไม่มีสิทธิ์เข้าถึง Leads/Deals หรือ Resource อื่น **อัปเดต 2026-09-23 (FR-CRM-123):** Marketing ได้สิทธิ์เทียบเท่า Sales Rep บน Leads/Deals แล้ว (ยกเว้น bulk/trash/restore/export และรายงาน ซึ่งยังเป็นของ Admin/Sales Manager เท่านั้น) |
...
- **Update verb — `PUT` vs `PATCH`**: as implemented, `PUT` marks a **full-replace** Update (every field is resent; an omitted field is cleared, not left unchanged) — Lead/Prospect/Company/Contact/Deal/Tag/Quote/Contract/User all use this. `PATCH` marks a **partial-update** Update (only the fields present in the body change) — Project/Product/Task/Settings and every `/admin/*` config-list resource (PipelineStage, LeadSource/ProspectSource/ProspectStage, the option-list resources, LeadScoringCriterion, NotificationRule, SalesTarget) use this instead. There's no deeper semantic split behind the two groups beyond how each handler was actually implemented — check the specific resource's own section below (or its handler's godoc) rather than assuming from this table alone.
...
| `DELETE` | `/prospects/:id` | Admin, Marketing, Sales Manager, Sales Rep | 🟢 | Soft-delete (§1.6) — recoverable via Trash/Restore below. |
| `GET` | `/prospects/trash` | Sales Manager/Admin only | 🟢 | List soft-deleted prospects, paginated like `GET /prospects`. Note: narrower than the read/write role set above — Marketing and Sales Rep don't get Trash/Restore/bulk-*, same split Lead doesn't have (Lead's bulk-*/Trash/Restore are Sales-Manager/Admin-only too, but Lead's read/write set is every Sales role, not a 4-role allowlist). |
...
| Method | Path | Status | Description |
|---|---|---|---|
| `GET` | `/activities` | 🟢 | Filters: `related_type`, `related_id` (required together), `type`. Backs the timeline on Deal/Company/Contact detail pages. **2026-09-25 (server-side paging for `/crm/activities`):** `related_type` may now be sent alone (every activity on that kind of record; `related_id` without `related_type` is still a 400); new `search` (case-insensitive literal substring on subject/notes **or** the linked record's display name). New `include_stage_changes=true` interleaves Deal stage-change history (the `deal`/`stage_changed` audit rows `GET /audit-log` already exposes to every pipeline role) into the same list via a SQL `UNION ALL`, so it is filtered, counted, sorted (`created_at`, newest first by default) and paged server-side. Those rows come back as `{ kind: "stage_change", type: "stage_change", related_type: "deal", related_id: <deal id>, from_stage, to_stage, created_by, created_at }`; real activities get `kind: "activity"` (ids are unique per kind only — key rows by kind+id). `type=stage_change` narrows to stage rows, any other `type` or a `related_type` other than `deal` drops them, and `search` matches a stage row's from/to stage or its Deal's title. Ignored (plain list returned) for a role outside the audit-log gate (e.g. Production). Without the flag the response is exactly the plain Activity list, so detail-page timelines are unaffected. |
...
| `GET` | `/reports/prospect-source-conversion?assigned_to=&date_from=&date_to=` | **Added 2026-09-03.** `FR-CRM-107`. Structural mirror of the row above, one funnel stage earlier — `[{ source, total, converted, conversion_rate }]`, "converted" = `status = 'Converted'`. **Admin/Marketing/Sales Manager/Sales Rep only** — a separate route group from the Admin/Sales-Manager-only `reports` group every other report lives under (`internal/routes/routes.go`'s `prospectReports`, the same role set as `/prospects` itself), since Marketing owns this funnel but has no access to any Deal/Lead report. `.../export` streams CSV, same pattern as every other report. **Refactored 2026-09-03 (later)** — the two "group by source, count a success condition" queries behind this row and the one above it were identical apart from which model/success-status, so both now call a shared `fetchSourceConversion(db, c, model, successCondition)` + `conversionRate(total, successful)` in `reports.go`, with each report's own public function just relabeling the generic `{source, total, successful}` row into its own typed shape (`Qualified` vs `Converted`). No behavior change. |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| `GET` | `/audit-log` | Admin, Sales Rep, Sales Manager | Filters: `entity_type`, `entity_id`, `actor_id`, date range — full, unrestricted use of all of these is **Admin-only**. Must be **append-only** at the storage layer (`NFR-007`) — no `PUT`/`DELETE` route should exist for this resource at all. **2026-09-25:** `action` (e.g. `stage_changed`) is now honored for every role — it narrows within the role's visible slice and can never widen it. It was previously ignored, so the Activities pages' `action=stage_changed` call also got back every other Deal audit row for an Admin (and `reassigned` rows for a Sales Manager), none carrying `after.stage`, which rendered as a bare "Stage set:" with nothing after the colon. |
...
| `PATCH` | `/admin/settings` | Admin | Body: `{ quarterly_sales_target: number, annual_revenue_goal: number }`. **Both fields are required on every request** — this is a singleton config row, not a per-field partial-update resource, so an omitted field is a `422` rather than "leave it unchanged." Both must be `>= 0`. Writes an audit-log entry (`entity_type: "settings"`, `action: "updated"`) only when at least one value actually changed — a no-op PATCH (identical values) doesn't add a second entry. **Updated 2026-09-24 (FR-CRM-124):** also optional `weekly_digest_enabled: boolean` (default `true`; omitting it leaves it unchanged). The row also carries server-set `last_weekly_digest_at` (read-only). |
| `GET` | `/admin/weekly-digest/preview` | Admin | **Added 2026-09-24 (FR-CRM-124).** Renders the weekly Overview Pipeline email as it would go out now, without sending: `{ subject, body, recipients: string[], week_from, week_to, enabled, smtp_configured, last_sent_at }`. |
| `POST` | `/admin/weekly-digest/test` | Admin | **Added 2026-09-24 (FR-CRM-124).** Emails the current digest to the calling Admin only (`{ sent_to }`); `422` when SMTP isn't configured or the Admin has no email. Doesn't affect the Monday schedule. **Scheduled send (2026-09-25):** from Monday 08:00 server-local time (`TZ`), the hourly check claims the week with one conditional update before sending, so a second API instance can't send it again; the claim is released only when every send failed, so the next hour retries. A settings save never touches `last_weekly_digest_at`. Money and percentages are formatted like the page (`฿1.6K`, rounded cohort %). |
...
| `GET` / `POST` | `/admin/company-sizes` | List / create a `CompanySizeOption` for `Company.size`. No prior hardcoded list — the seeded rows (`1-10 คน` … `1000+ คน`, plus `> 100 คน`, updated 2026-09-22 to carry a "คน"/people unit) are a tunable starting point, not a fixed business rule. The seed only fills an **empty** table, so **2026-09-23** added a one-time startup migration (`database.MigrateCompanySizeDefaults`) for databases seeded earlier: it renames the old unit-less defaults (`1-10` … `1000+`) to their "… คน" names, repoints every Company using them in the same transaction (`Company.size` stores the option name, and Create/Update reject an inactive/unknown one), and adds `> 100 คน`. It's skipped once `> 100 คน` exists in any state, so an Admin's later renames aren't undone; custom sizes and values that were never a default (e.g. a legacy `50-100`) are left alone. List order is `name ASC` as text; the frontend re-sorts by headcount (`stores/companySizeOptions.ts`). |
...
| `GET` / `POST` | `/admin/prospect-stages` | List / create a `ProspectStage` for `Prospect.status` (§3a) — replaces the previously hardcoded `ProspectStatus` *working*-stage enum (`New/Engaging/Nurturing/Disqualified`) as the source of truth. `"Converted"` is deliberately **not** a row here — it's the system-set terminal status `POST /prospects/:id/convert` alone sets (see the `ProspectStatus` note above), so Create/Update reject a client-supplied `name: "Converted"` with `422`. No `is_won_stage`/`is_lost_stage` equivalent — Prospect stages are a straight funnel sequence, not a win/loss outcome — but `is_disqualified_stage` mirrors that pattern for the one stage that does gate behavior (the frontend's "Convert to Lead" action visibility/status badge color, and — added same day — `checkProspectStaleRule`'s FR-CRM-107 exclusion), so renaming "Disqualified" doesn't silently break any of the three. Added 2026-09-09. **Updated 2026-09-24:** optional `stale_days` (1–365, or `null` for the default 14) — how long a record may sit in this stage before the Overview Pipeline flags it stale; not used on won/lost/disqualified stages. Unlike the stage's other fields, an update that omits `stale_days` leaves the saved value alone (older clients can't wipe it); only an explicit `null` resets it. **Updated 2026-09-24:** a rename repoints Prospects' `status`/`previous_stage` in the same transaction (see pipeline stages). |
| `PATCH` / `DELETE` | `/admin/prospect-stages/:id` | Update (including `is_active`/`sort_order`/`is_disqualified_stage`) / soft-delete (`is_active: false`, not a hard row delete — existing Prospects may still reference the name). Same reserved-name rejection as Create. **Fixed 2026-09-10**: setting `is_disqualified_stage` now clears that flag from every other row (one transaction alongside the save), same fix and reasoning as `is_won_stage`/`is_lost_stage` above — `checkProspectStaleRule` resolves "the" disqualified stage with a single lookup. |
...
| Method | Path | Status | Description |
|---|---|---|---|
| `GET` | `/dashboard/summary` | 🟢 | Query params mirror the dashboard's filter bar exactly: `date_from`, `date_to` (or a `period` preset: `all\|month\|quarter\|year\|last6\|last12`), `business_unit`, `business_unit_item`, `channel`. Returns every stat card + chart the page renders in one response (shape below). Responses are cached server-side per exact query-param combination for up to 30s, so a write immediately followed by a refresh may briefly still show pre-write numbers — acceptable staleness for a stat-card dashboard, but worth knowing if a future test asserts read-after-write freshness here specifically. **Fixed 2026-09-10**: a malformed `date_from`/`date_to` (not a `YYYY-MM-DD` string) now `422`s instead of reaching Postgres as a raw query bound and silently degrading every figure to zero. |
| `GET` | `/dashboard/prospect-summary` | 🟢 | **Added 2026-09-03, FR-CRM-107.** Marketing's own dashboard tab — deliberately a separate endpoint/handler (`internal/handlers/dashboard_prospect.go`), not folded into the Deal-centric `Summary` above, since Prospect volume doesn't warrant that endpoint's 30s response cache. Query params: `assigned_to`, `date_from`, `date_to` (no `period` preset, no `business_unit`/`channel` — those are `Summary`'s own Deal-side filters). Response: `{ total_prospects, open_prospects, converted_count, conversion_rate, status_breakdown: [{ status, count }], source_breakdown: [{ source, total, converted, conversion_rate }] }` — `source_breakdown` reuses `ReportHandler.fetchProspectSourceConversion` directly (same cross-handler-reuse pattern `Summary` already uses for `fetchSalesCycle`). Not `RequireRoles`-gated at the route, same openness as `/dashboard/summary` itself — the frontend decides which role sees which dashboard tab. |
| `GET` | `/pipeline/overview` | 🟢 | **Added 2026-09-23, FR-CRM-123.** One-request payload for the Overview Pipeline page (built by `internal/overview`, which the weekly digest also uses; `internal/handlers/pipeline_overview.go` only parses the query). Day boundaries ("today", period dates) are in the server's local time — `TZ`, `Asia/Bangkok` in the Docker image. Gated to `salesPipelineRoles` (Admin/Sales Rep/Sales Manager/Marketing; Production 403s). Params: `date_from`/`date_to` (`YYYY-MM-DD`, both inclusive; default the last 7 days incl. today; malformed or reversed → `422`), `assigned_to` (user id or `unassigned`), `source` (Prospect/Lead `source`, Deal `channel`), `business_unit`, `tag`, `search` (record name/title or linked Company name), `card_limit` (cards returned per lane, default 30, max 100 — lane `count`/`value` are always exact). Response: `{ period: { date_from, date_to, prev_date_from, prev_date_to }, summary: { new_prospects, new_leads, new_deals: { current, previous }, won: { current, previous, value, previous_value }, open_pipeline: { count, value, weighted_value } }, zones: [{ key: 'prospect'|'lead'|'deal', lanes: [{ name, kind: 'open'|'won'|'lost'|'converted'|'other', terminal, count, value, cards: [{ id, name, company_id, company_name, assigned_to, source, value, probability, lost_reason, from_prospect, stage_entered_at, created_at, stage }] }] }] }`. `previous` is the equal-length window immediately before the selected one. Lanes follow the configured stage lists (active `ProspectStage` rows + the reserved `Converted`; the fixed Lead statuses; active `PipelineStage` rows), flagged terminal by `is_disqualified_stage`/`is_won_stage`/`is_lost_stage`. **Open lanes show every current record regardless of period** (longest-in-stage first); **terminal lanes show only records whose `stage_entered_at` falls inside the period** (most recent first). **Updated 2026-09-23:** a converted Lead (`converted_deal_id` set) sits in the Lead zone's own terminal `Converted` lane for the period it converted in (mirroring Prospect's `Converted` one stage earlier), instead of disappearing from the zone; its lane is derived (`CASE WHEN converted_deal_id IS NOT NULL THEN 'Converted' ELSE status END`) since conversion leaves `status` at Qualified, and conversion now always restamps `stage_entered_at` (existing converted Leads were backfilled to their Deal's `created_at`). Each zone also gets a trailing `kind: 'other'` lane (`name: ""`, never terminal, only present when non-empty) for records whose stage/status isn't one of the zone's lanes — blank, or a stage an Admin deactivated/renamed — so they stay visible instead of dropping off the board while still counting in the summary; each card's `stage` carries its raw value. `won` counts `deals.status = 'won'` with `stage_entered_at` in the window, so a renamed Won stage still counts; `weighted_value` = Σ `value × probability / 100` over open Deals. Not cached. Query shape: per zone, one grouped count/value query and one `ROW_NUMBER() OVER (PARTITION BY lane)` card query; the summary is one `COUNT(*) FILTER (...)` query per entity for current/previous plus one each for Won and open pipeline — about 13 queries per load however many stages are configured. **Updated 2026-09-24 (review accuracy):** the response adds `highlight: { stale, moved, slipped, stale_deals, stale_deal_value }` — exact counts over every open-lane record (open lanes plus `other`), not only the cards returned, so the page's highlight buttons and stale-Deals banner can't undercount a lane past `card_limit`; `stale` uses each lane's own threshold, `moved` = entered its lane inside the period and not merely created there, `slipped` = a move backward. Each open lane carries `stale_days` (its stage's `stale_days`, else 14; Lead statuses always 14; 0 on terminal lanes). Each card carries `previous_stage` (the lane it left on its last move, "" if never moved) and `direction` (`forward`/`backward` in the zone's lane order; won/converted rank above every open lane, lost lanes have no rank, so leaving Lost or a retired stage gives ""). `summary.conversion: { prospect_to_lead, lead_to_deal, deal_to_won }` each `{ cohort, converted }` — of the records created in the period, how many have reached the next step so far (Prospect `converted_lead_id` set, Lead `converted_deal_id` set, Deal `status = won`); unlike comparing two `new_*` counts it can't exceed 100%. |
