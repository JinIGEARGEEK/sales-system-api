...
| `POST` | `/admin/weekly-digest/test` | Admin | **Added 2026-09-24 (FR-CRM-124).** Emails the current digest to the calling Admin only (`{ sent_to }`); `422` when SMTP isn't configured or the Admin has no email. Doesn't affect the Monday schedule. |
