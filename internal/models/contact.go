...
type Contact struct {
	...
	// index — mirrors Company.Status's own index: List/Export both filter on
	// this column (applyContactFilters), same as Company's, so it shouldn't
	// be the one AuditedModel resource among the two left doing a sequential
	// scan for that filter as the table grows.
