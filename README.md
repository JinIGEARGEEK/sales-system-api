...
- [`biz_spec/api-system-spec.md`](biz_spec/api-system-spec.md) is the living API contract — keep it in sync with the code in the same PR that changes a request/response shape or adds an endpoint, and add a [`CHANGELOG.md`](CHANGELOG.md) entry alongside it.
